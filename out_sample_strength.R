...
for (signal_strength in signal_strength_values) { 
  ...
  for (sim_id in 1:n_simulations) {      
    ...
    full_data <- gen_toy_data(  
      ...
      data_sd = 1,  
      signal_mean = 0,  
      signal_sd = sqrt(signal_strength),  # 使用当前信号强度的平方根  
      seed_id = as.integer(signal_strength * 10 + sim_id)   # 更新种子  
    )  
    ...
    set.seed(as.integer(signal_strength * 10 + sim_id + 100))  # 加100与上面的种子区分  
    ...
    if (sim_id %% 10 == 0) {  
      temp_file <- paste0("strength_results_", signal_strength, "_partial_", sim_id, ".csv")  
      write.csv(all_results, temp_file, row.names = FALSE)  
    }  
  }  
}  

write.csv(all_results, "out_sample_strength_20.csv", row.names = FALSE)