...
num_groups <- 10  
alpha_values <- c(0.95)  
signal_sparsity_values <- c(0.2, 0.4, 0.6, 0.8) 
n_simulations <- 100  
train_prop <- 0.8  
correlation <- 0.5  
...
for (signal_sparsity in signal_sparsity_values) { 
  
  for (sim_id in 1:n_simulations) {      
    cat("\n=== Testing signal sparsity:", signal_sparsity, "iteration:", sim_id, "===\n")   

    groups <- rep(1:num_groups, each = p/num_groups)  

    set.seed(1000 * as.numeric(signal_sparsity) + sim_id)   
    full_data <- gen_toy_data(  
      ...
      signal_mean = 0,  
      signal_sd = sqrt(10),  
      seed_id = 1000 * as.numeric(signal_sparsity) + sim_id   
    )  
    ...
    simulation_results$signal_sparsity <- signal_sparsity  
    simulation_results$iteration <- sim_id

    all_results <- rbind(all_results, simulation_results)

    if (sim_id %% 10 == 0) {
      temp_file <- paste0("features_results_signal_sparsity_partial_", signal_sparsity, "_", sim_id, ".csv")
      write.csv(all_results, temp_file, row.names = FALSE)
    }
  }
}

write.csv(all_results, "elastic_net_sparsity.csv", row.names = FALSE)
