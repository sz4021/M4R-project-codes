...
for (correlation in correlation_values) { 
  ...
  for (sim_id in 1:n_simulations) {      
    ...
    simulation_results$correlation <- correlation  
    simulation_results$iteration <- sim_id

    all_results <- rbind(all_results, simulation_results)

    if (sim_id %% 10 == 0) {
      temp_file <- paste0("features_results_correlation_partial_", correlation, "_", sim_id, ".csv")
      write.csv(all_results, temp_file, row.names = FALSE)
    }
  }
}

write.csv(all_results, "elastic_net_correlation.csv", row.names = FALSE)