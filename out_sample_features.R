...
p_values <- c(300, 400, 500, 600, 700)
n <- 100  
num_groups <- 10  
...
for (p in p_values) { 
  ...
  for (sim_id in 1:n_simulations) {      
    cat("\n=== Testing features:", p, "iteration:", sim_id, "===\n")   
 
    groups <- rep(1:num_groups, each = p/num_groups)  
 
    set.seed(100 * (p %% 100) + sim_id)  
    ...
    if (sim_id %% 10 == 0) {  
      temp_file <- paste0("features_results_", p, "_partial_", sim_id, ".csv")  
      write.csv(all_results, temp_file, row.names = FALSE)  
    }  
  }  
}  

write.csv(all_results, "out_sample_features_700.csv", row.names = FALSE)