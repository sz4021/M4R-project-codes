...
for (signal_sparsity in signal_sparsity_values) {  
  ...
  for (sim_id in 1:n_simulations) {      
    ...
    set.seed(2000 * as.numeric(signal_sparsity) + sim_id) 
    ...
    if (sim_id %% 10 == 0) {  
      temp_file <- paste0("sparsity_results_", signal_sparsity, "_partial_", sim_id, ".csv")  
      write.csv(all_results, temp_file, row.names = FALSE)  
    }  
  }  
}  
 
write.csv(all_results, "out_sample_sparsity.csv", row.names = FALSE)